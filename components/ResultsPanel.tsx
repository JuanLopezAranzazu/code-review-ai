...
const TAB_TRIGGER_CLASS =
  "shrink-0 whitespace-nowrap rounded-[var(--radius-sm)] px-3 py-1.5 text-sm text-[var(--color-text-muted)] transition-colors hover:text-[var(--color-text)] data-[state=active]:bg-[var(--color-brand-muted)] data-[state=active]:text-[var(--color-brand-hover)]";

export function ResultsPanel({ analysis }: { analysis: CodeAnalysis }) {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[var(--color-border)] p-2.5 sm:p-5">
        <ScoreGauge score={analysis.score} />
      </div>

      <Tabs.Root defaultValue="resumen" className="flex min-h-0 flex-1 flex-col">
        <Tabs.List className="flex gap-1 overflow-x-auto border-b border-[var(--color-border)] px-3 pt-2">
